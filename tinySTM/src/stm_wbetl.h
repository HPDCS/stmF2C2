...
static INLINE int
stm_wbetl_validate(stm_tx_t *tx,int committing)
{
  r_entry_t *r;
  int i;
  stm_word_t l;
...
# endif /* UNIT_TX */
            ...
            _tinystm.conflict_cb(tx->attr.id, other->attr.id);
# ifdef UNIT_TX
          }
# endif /* UNIT_TX */
        }
...
# endif /* ! IRREVOCABLE_IMPROVED */
  ...
  if (unlikely(tx->start != t - 1 && !stm_wbetl_validate(tx,1))) {
    /* Cannot commit */
#if CM == CM_MODULAR
    /* Abort caused by invisible reads */
    tx->visible_reads++;
...
# endif /* CM == CM_MODULAR */
    	  ...
    	  *(w->lock + LOCK_ARRAY_SIZE)=tx->attr.id;
